...
def render_seam_strip(results_by_key: dict) -> Path:
    ...
    step_mm = 5.8  # DESIGN.md-adjacent vertical step at the seam (lid rides
    # in through-slots above the front-wall top; the top panel sits at
    # TOP_PANEL_Z0 -- drawn here as a simple visual offset marker, not a
    # precise re-derivation of the through-slot geometry.)
    gap_px = 40
    ...
    draw.text((10, h - 40), "Lid + Top Panel seam-continuity check "
    ...
    out_path = OUT_DIR / "lid_top_panel_seam.png"
...
def write_proofs_md(results: list[RenderResult], seam_path: Path, contact_path: Path) -> None:
    lines = []
    ...
    lines.append("## Grip-capsule verdict (Sliding Lid)\n")
    lines.append(
    ...
    lines.append(
        "**Separate finding -- front-edge content gap**: measured directly on the "
        "CCW-rotated art (connected ink bounding box), drawn content only starts at "
        "depth ~16.2mm and runs to ~78.9mm (of the 0-79mm lid) -- i.e. the front-most "
        "~16mm of the lid's depth is BLANK (no ink at all), while the rear edge is inked "
        "almost flush (0.1mm margin). This is a genuine property of the stored "
        "`sliding_lid.png` file (confirmed independently of the rotation-direction "
        "question above), not a compositing artifact -- visible as a plain white band "
        "along the lid's front edge in `sliding_lid.png`'s proof. Combined with the grip "
        "-slot misalignment above, the entire front ~25mm of the lid (grip-slot zone "
        "included) is either blank or only partially inked -- worth Ben's eyes before "
        "sign-off.\n"
    )
...
def main() -> None:
    ...
    write_proofs_md(results, seam_path, contact_path)
