...
def render_variant_b_seam() -> Path | None:
    """Rev-3: minimal seam-continuity proof for the head-anchored-vs-centered
    trim-judgment B variant (sliding_lid_VARIANT_B.png / top_panel_VARIANT_B.png
    -- see art_postprocess.py's process_4231 step 8). These files have no
    PartSpec / cut-geometry overlay (they're a trim-judgment comparison, not
    a candidate to actually cut) -- reuses load_art()'s rotation convention
    and _paste_seam_canvas() directly rather than the full render_part()
    geometry pipeline, per BRIEF's 'minimal code' instruction."""
    lid_path = ART_DIR / "sliding_lid_VARIANT_B.png"
    panel_path = ART_DIR / "top_panel_VARIANT_B.png"
    ...
    return _paste_seam_canvas(
        ...
        "VARIANT B (centered-window) seam-continuity check -- trim-judgment "
        "comparison only, not a cut candidate; canonical A files "
        "(sliding_lid.png/top_panel.png, head-anchored) are what "
        "art_proofs.py's main per-part renders use.",
        "lid_top_panel_seam_VARIANT_B.png")
...
def write_proofs_md(results: list[RenderResult], seam_path: Path, contact_path: Path,
                     variant_b_seam_path: Path | None = None) -> None:
    lines = []
    ...
    if variant_b_seam_path is not None:
        lines.append("## Rev-3 lid/top-panel VARIANT B (trim-judgment comparison)\n")
        lines.append(
            "`art_postprocess.py`'s process_4231 (rev 3) produces a second, "
            "CENTERED-window pair -- `sliding_lid_VARIANT_B.png` / "
            "`top_panel_VARIANT_B.png` -- alongside the canonical "
            "head-anchored A pair, so Ben can compare trim judgments at this "
            "gate. B has no PartSpec / cut-geometry overlay (it's not a cut "
            "candidate); its comparison proof is a minimal seam-continuity "
            f"strip only: `{variant_b_seam_path.name}`. See "
            "art/engrave/PLACEMENTS.md's sliding_lid.png entry ('VARIANT B') "
            "for the exact window numbers and what each variant trims.\n"
        )

    lines.append("## Files\n")
    lines.append(f"- `{seam_path.name}` -- lid+top-panel seam continuity strip.")
    if variant_b_seam_path is not None:
        lines.append(f"- `{variant_b_seam_path.name}` -- VARIANT B (centered-window) "
...
def main() -> None:
    ...
    print("Rendering VARIANT B seam strip (if present)...")
    variant_b_seam_path = render_variant_b_seam()
    ...
    write_proofs_md(results, seam_path, contact_path, variant_b_seam_path)
