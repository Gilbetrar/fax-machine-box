...
| Wall base plane | Z = 3.175 | walls stand **on** the bottom panel |
| Full wall height | 123.825 | 127 − T |
...
|---|---|---|
| Bottom panel | 0 → 3.175 | full 304.8 × 165.1 footprint |
...
- Blank: **304.8 × 165.1** (full footprint, exact; edges plain).
- Finger holes for each wall's bottom-edge fingers, centered on the wall
  midplanes: front wall at X = 1.5875, rear wall at X = 303.2125, side walls
  at Y = 1.5875 and Y = 163.5125, and the divider at X = 80.9625 (hole line
  spanning Y = 3.175 → 161.925).

### 2. Side walls — 2× (left Y=0→3.175, right Y=161.925→165.1)

- Nominal: **298.45 (X) × 123.825 (Z)**, standing on the bottom panel.
- Vertical edges: finger joints to front wall (only up to Z=118.025 on the
  ...
  wall (full height).
- Bottom edge: fingers down into bottom panel holes.
- Top edge: plain over the paper compartment/rail; finger-jointed to the top
  panel over the bay (X = 82.55 → 301.625).
- **Lid through-slot**: X = 0 → 79.375 (open at front edge), Z = 118.025 →
  122.0. Leaves a 5.0mm rail above (Z 122 → 127) that is cantilevered from
  X = 79.375 — handle gently until assembled.
- **Divider finger holes**: vertical line, X = 79.375 → 82.55, Z = 3.175 → 123.825.
- **Shelf finger holes**: horizontal line, Z = 61.9125 → 65.0875, X = 82.55 → 301.625.
- **Right wall only**: "FAX MACHINE" engraving (red), 5×7 pixel font,
  pixel size 4.0mm → text ≈ 240 × 28, centered at X = 152.4, Z = 63.5.
  Engrave on the exterior face — mind part mirroring when generating.

### 3. Front wall — 1×

- Nominal: **158.75 (Y) × 114.85 (Z)** (Z = 3.175 → 118.025).
- Bottom edge fingers into bottom panel; vertical edges finger-joint to side
  walls; top edge plain (the lid slides over it).

### 4. Rear wall — 1×

- Nominal: **158.75 (Y) × 123.825 (Z)**, full height.
- Bottom → bottom panel; sides → side walls; top edge → top panel.
- **Two drawer openings**, each **152.4 wide (= W_INT − 2T) × 55.0 tall**,
  ...
  - Bottom opening: Z = 3.175 → 58.175 (sill-free: bottom edge = bay floor).
  - Top opening: Z = 65.0875 → 120.0875 (sill-free: bottom edge = shelf top).
  - Webs remaining: 3.7375 below the shelf line and 3.7375 above the top
    opening — symmetric, each ≥ T; do not enlarge openings.

### 5. Vertical divider — 1×

- Nominal: **158.75 (Y) × 120.65 (Z)** (Z = 3.175 → 123.825).
- Bottom edge fingers into bottom panel holes; side edges finger into
  side-wall hole lines; top edge fingers up into top panel holes. Fully
  captive on 4 edges → carries the structural load the SPEC requires.
...
- **Finger notch**: half-round, R = 15 (30 wide), cut into the **top edge** of
  BOTH the faceplate and the body's front wall, aligned, so a finger reaches
  through into the drawer. Centered in Y.
