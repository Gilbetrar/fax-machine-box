...
- Boxes.py's `FingerJointSettings` `play` parameter widens finger holes/slots symmetrically around their nominal position. On a hole row that sits near a part's edge, that symmetric widening pushes the hole boundary past the part edge by `play/2` — sub-kerf, harmless, and it's why the SVG harness clusters bbox containment with a 0.2mm tolerance instead of an exact edge match.
- When a fingered edge on one part must align with a `fingerHolesAt` row on another part, the fingered *segment*'s length has to equal the mating row's length exactly — Boxes.py lays fingers out per-segment, so a full-edge `'f'` edge (spanning the whole part edge) will misalign against a shorter hole row. Use `edges.CompoundEdge` to restrict the finger segment to just the mating span.
- SVG `<text>` fill color is a real laser instruction, not just a visual label — a `<text>` element colored engrave-red will get engraved. Reference-only labels (part names, sheet numbers) must use a color the cut/engrave driver is told to ignore (this project uses gray), never the engrave color.
